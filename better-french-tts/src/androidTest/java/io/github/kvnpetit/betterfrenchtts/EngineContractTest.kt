...
class EngineContractTest {
    @Test
    fun exportDestinationExceptionDoesNotLeaveTheInstanceBusy() =
        fixture(nativeConfig()) { speech, engine ->
            engine.fileFailure = SecurityException("Destination denied")
            runBlocking {
                assertTrue(
                    withTimeout(3000) { speech.synthesizeToFileAndAwait("bonjour", exportFile()) }
    ...
    @Test
    fun rejectedAwaitedExportDoesNotInterruptActiveSpeech() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            ...
            runBlocking {
                assertTrue(
                    withTimeout(3000) { speech.synthesizeToFileAndAwait("export", exportFile()) }
                        is SpeechResult.Error
                )
            }
            drain()
    ...
    @Test
    fun oversizedSsmlExportIsRejectedBeforeDispatch() = fixture { speech, engine ->
        assertTrue(speech.synthesizeToFile("&".repeat(4000), exportFile()) is SpeechResult.Error)
        assertTrue(engine.requests.isEmpty())
    }

    @Test
    fun restartingAnActiveQueueKeepsAllItemsInEitherMode() {
        for (mode in BetterFrenchTts.PlaybackMode.entries) fixture(
            nativeConfig().copy(playbackMode = mode)
        ) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun queuedDslValidationFailureLeavesNoActiveQueue() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun failureOfAnEarlySsmlChunkStopsTheQueue() = fixture { speech, engine ->
        engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun completePreviewMatchesNativeEngineInputWithAliases() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun unsupportedNativePreviewReportsAnErrorWithoutSpeaking() =
        fixture(nativeConfig()) { speech, engine ->
    ...
    @Test
    fun detailedErrorsPreserveAndroidCodeAndIdentity() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            ...
            runBlocking {
                val result =
                    async(start = CoroutineStart.UNDISPATCHED) { speech.speakAndAwait("bonjour") }
    ...
    @Test
    fun awaitedExportWaitsForFileCompletionInEitherMode() {
        for (mode in BetterFrenchTts.PlaybackMode.entries) fixture(
            nativeConfig().copy(playbackMode = mode)
        ) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            runBlocking {
                val export =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        speech.synthesizeToFileAndAwait("bonjour", exportFile())
                    }
                drain()
    ...
    @Test
    fun awaitedExportReturnsImmediateAndAsynchronousErrors() =
        fixture(nativeConfig()) { speech, engine ->
            runBlocking {
                assertTrue(
                    withTimeout(3000) { speech.synthesizeToFileAndAwait("bonjour", exportFile()) }
                ...
                assertEquals(
                    ...
                    (withTimeout(3000) { export.await() } as SpeechResult.Error).engineCode,
                )
            }
        }

    @Test
    fun cancellingExportStopsItAndAllowsTheNextRequest() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            runBlocking {
                val export =
                    async(start = CoroutineStart.UNDISPATCHED) {
                        speech.synthesizeToFileAndAwait("export", exportFile())
                    }
                drain()
    ...
    @Test
    fun stopCompletesAwaitedExportWithError() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            runBlocking {
                val export =
    ...
    @Test
    fun voiceCannotChangeDuringPlayback() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            speech.speak("bonjour")
            val previous = speech.currentVoice
            assertTrue(
                speech.trySetVoice(Voice("new", Locale.FRANCE, 500, 100, false, emptySet()))
                    is SpeechResult.Error
            )
    ...
    @Test
    fun deniedFocusDoesNotSendSpeechInEitherMode() {
        for (mode in BetterFrenchTts.PlaybackMode.entries) {
            val focus = Focus().apply { granted = false }
            fixture(BetterFrenchTts.Config(playbackMode = mode), focusAccess = focus) {
                speech,
                engine ->
                engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun nativeSegmentsKeepOneFocusLeaseUntilCompletion() {
        val focus = Focus()
    ...
    @Test
    fun focusLossStopsWithoutAutomaticRestartAndIgnoresStaleEvents() {
        val focus = Focus()
        fixture(BetterFrenchTts.Config(), focusAccess = focus) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            runBlocking {
    ...
    @Test
    fun transientFocusLossCanPreserveQueueForManualResume() {
        val focus = Focus()
        fixture(
            BetterFrenchTts.Config(
                focusLossBehavior = BetterFrenchTts.FocusLossBehavior.PAUSE_QUEUE
            ),
            focusAccess = focus,
        ) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun lateStartAndRangeCallbacksAfterStopAreIgnored() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            var callbacks = 0
    ...
    @Test
    fun fileExportCannotChangeActivePlaybackControls() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            speech.speak("bonjour")
            val file =
    ...
    @Test
    fun fileCompletionIsDeliveredToCaller() {
        fixture(BetterFrenchTts.Config(audioFocus = BetterFrenchTts.AudioFocusMode.NONE)) {
            speech,
            engine ->
            engine.response = TextToSpeech.SUCCESS
            var completed = false
    ...
    @Test
    fun exactFrancePolicyRejectsRegionalFallback() {
        fixture(
            BetterFrenchTts.Config(
            ...
            prepare = {
                it.offeredVoices =
                    setOf(Voice("ca-local", Locale.CANADA_FRENCH, 500, 100, false, emptySet()))
            },
        ) { speech, _ ->
    ...
    @Test
    fun disabledNormalizationPreviewPreservesInput() {
        fixture(
            BetterFrenchTts.Config(
    ...
    @Test
    fun rejectedSsmlQueueDoesNotRemainActive() {
        fixture { speech, _ ->
    ...
    private class Engine(context: Context) : TextToSpeech(context, null) {
        ...
        override fun setVoice(voice: Voice): Int {
            if (voiceResult == SUCCESS) selectedVoice = voice
            return voiceResult
        }
    ...
    private fun fixture(
        config: BetterFrenchTts.Config =
            BetterFrenchTts.Config(
                audioFocus = BetterFrenchTts.AudioFocusMode.NONE,
                playbackMode = BetterFrenchTts.PlaybackMode.SSML,
            ),
        prepare: (Engine) -> Unit = {},
        ...
        instrumentation.runOnMainSync {
            ...
            speech =
                BetterFrenchTts(instrumentation.targetContext, config, focusAccess) {
                    context,
                    listener ->
    ...
    private fun nativeConfig() =
        BetterFrenchTts.Config(audioFocus = BetterFrenchTts.AudioFocusMode.NONE)
    ...
    @Test
    fun nativeSpeechUsesPlainTextAndDictionaryAliases() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            speech.addPronunciation(PronunciationRule.Alias("Huawei", "Oua-ouei"))
    ...
    @Test
    fun nativeProsodyWaitsForPreviousSegmentAndUsesRealSilence() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            speech.speak {
                slow { text("lent") }
    ...
    @Test
    fun flushCompletesInterruptedNativeCoroutine() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
            runBlocking {
                val pending =
                    async(start = CoroutineStart.UNDISPATCHED) { speech.speakAndAwait("premier") }
                speech.speak("second")
                assertTrue(withTimeout(3000) { pending.await() } is SpeechResult.Error)
            }
    ...
    @Test
    fun engineOnStopCompletesAwaitWithoutDoneOrError() = fixture { speech, engine ->
        engine.response = TextToSpeech.SUCCESS
        runBlocking {
            val pending =
                async(start = CoroutineStart.UNDISPATCHED) { speech.speakAndAwait("premier") }
    ...
    @Test
    fun strictOfflineInitializationRejectsMissingOrNetworkOnlyVoices() {
        for (voices in
            listOf(emptySet(), setOf(Voice("online", Locale.FRANCE, 500, 100, true, emptySet())))) {
    ...
    @Test
    fun regionalVoiceWinsOverHigherQualityDifferentRegion() =
        fixture(
            nativeConfig().copy(locale = Locale.CANADA_FRENCH),
            {
                it.offeredVoices =
                    setOf(
                        Voice("fr-high", Locale.FRANCE, 500, 100, false, emptySet()),
                        Voice("ca-local", Locale.CANADA_FRENCH, 300, 100, false, emptySet()),
                    )
            },
        ) { speech, _ ->
            assertEquals("ca-local", speech.currentVoice?.name)
        }

    @Test
    fun uninstalledOfflineVoicesAreNotSelected() =
        fixture(
            nativeConfig(),
            {
                it.offeredVoices =
    ...
    @Test
    fun rejectedManualVoiceDoesNotChangeReportedVoice() =
        fixture(nativeConfig()) { speech, engine ->
            val before = speech.currentVoice
            engine.voiceResult = TextToSpeech.ERROR
            assertTrue(
                speech.trySetVoice(Voice("other", Locale.FRANCE, 300, 100, false, emptySet()))
                    is SpeechResult.Error
            )
    ...
    @Test
    fun pausedQueueIgnoresOldCompletionAfterResume() =
        fixture(nativeConfig()) { speech, engine ->
            engine.response = TextToSpeech.SUCCESS
    ...
    @Test
    fun failedNativeQueueDoesNotRemainPlaying() =
        fixture(nativeConfig()) { speech, _ ->
            assertTrue(speech.enqueue("test").playQueue() is SpeechResult.Error)
            assertFalse(speech.isQueuePlaying)
        }

    @Test
    fun synchronousRejectionIsReturnedForEverySpeechApi() = fixture { speech, engine ->
        assertTrue(speech.speak("bonjour") is SpeechResult.Error)
        assertTrue(speech.speak { text("bonjour".repeat(1000)) } is SpeechResult.Error)
        assertTrue(
            speech.speakWithPreset(SpeechPreset.NEUTRAL) { text("bonjour".repeat(1000)) }
                is SpeechResult.Error
        )
    ...
    @Test
    fun immediateEngineFailureDoesNotLeaveCoroutineSuspended() = fixture { speech, _ ->
        runBlocking {
            withTimeout(3000) {
    ...
    @Test
    fun shutdownCompletesPendingAwaitAndRejectsFurtherSpeech() = fixture { speech, engine ->
        engine.response = TextToSpeech.SUCCESS
        runBlocking {
            val pending =
                async(start = CoroutineStart.UNDISPATCHED) { speech.speakAndAwait("bonjour") }
    ...
    @Test
    fun longEscapedTextAndNestedDslRespectEngineLimit() = fixture { speech, engine ->
        engine.response = TextToSpeech.SUCCESS
        assertEquals(SpeechResult.Success, speech.speak("&😀".repeat(3000)))
        assertEquals(
            SpeechResult.Success,
            speech.speak { paragraph { slow { text("&😀".repeat(3000)) } } },
        )
    ...
    @Test
    fun oversizedAtomicAliasReturnsErrorInsteadOfFalseSuccess() = fixture { speech, engine ->
        engine.response = TextToSpeech.SUCCESS
