 ...
 *
 * The output is wrapped in an SSML `<speak>` root element. Interpretation of this markup depends on
 * the installed TTS engine; Android does not guarantee SSML support. All text content is
 * XML-escaped to prevent injection of unintended markup.
 *
 * This is an internal component — library consumers use
 * [io.github.kvnpetit.betterfrenchtts.BetterFrenchTts] which calls the renderer automatically.
 */
object SsmlRenderer {
     ...
     *
     * Each group can then be wrapped and rendered separately. This is used internally to chunk
     * DSL-generated SSML that exceeds the TTS engine's ~4 000 character limit.
     *
     * @param nodes The SSML nodes to split.
     * @param maxContentLength Maximum rendered length of the nodes in each group (excluding any
     *   outer wrapper like `<speak>` or `<prosody>`).
     * @return A list of node groups. If everything fits, returns a single-element list. Atomic
     *   pronunciation nodes are not split; an oversized one remains a single node so the caller can
     *   reject it without changing its meaning.
     */
    ...
    private fun splitNode(node: SsmlNode, limit: Int): List<SsmlNode> {
        ...
        fun wrap(parts: List<SsmlNode>): SsmlNode =
            when (node) {
    ...
    private fun renderNode(node: SsmlNode): String =
        when (node) {
            ...
            is SsmlNode.Emphasis ->
                "<emphasis level=\"${escapeXml(node.level)}\">${renderChildren(node.children)}</emphasis>"

            is SsmlNode.Phoneme ->
                "<phoneme alphabet=\"ipa\" ph=\"${escapeXml(node.ph)}\">${escapeXml(node.content)}</phoneme>"

            is SsmlNode.Sub ->
                "<sub alias=\"${escapeXml(node.alias)}\">${escapeXml(node.content)}</sub>"
    ...
    private fun escapeXml(text: String): String {
        ...
        if (
            text.indexOfFirst { it == '&' || it == '<' || it == '>' || it == '"' || it == '\'' } ==
                -1
        )
            return text
