...
internal object NativeSpeechPlan {
            ...
            for (node in items) when (node) {
                ...
                is SsmlNode.Phoneme ->
                    throw IllegalArgumentException(
                        "IPA requires explicitly selected SSML playback and a compatible engine"
                    )
                is SsmlNode.Break -> {
                ...
                is SsmlNode.Emphasis -> {
                    val multiplier =
                        when (node.level) {
                            ...
                            "moderate" -> 0.95f
                            "reduced" -> 1.05f
                            else -> throw IllegalArgumentException("Unknown emphasis")
                        }
                ...
                is SsmlNode.SayAs ->
                    text(
                        when (node.interpretAs) {
                            "cardinal" -> FrenchFormats.number(node.content, region)
                            "ordinal" ->
                                FrenchFormats.ordinal(node.content.toLong(), region = region)
                            ...
                            "characters" -> spell(node.content)
                                ...
                                throw IllegalArgumentException(
                                    "Unsupported native interpretation: ${node.interpretAs}"
                                )
                        },
                        style,
                    )
        ...
        java.text.Normalizer.normalize(text, java.text.Normalizer.Form.NFC)
            ...
            .joinToString(", ") { code ->
                if (code > Char.MAX_VALUE.code) "caractère unicode $code"
                    ...
                    FrenchCharMap.resolve(code.toChar())
    ...
    private fun rate(value: String): Float {
        val number =
            when (value) {
                ...
                "fast" -> 1.25f
                "x-fast" -> 1.5f
                else -> value.removeSuffix("%").toFloat() / 100
            }
    ...
    private fun pitch(value: String): Float {
        val number =
            if (value.endsWith("st")) 2.0.pow(value.removeSuffix("st").toDouble() / 12).toFloat()
            else
                when (value) {
                    "x-low" -> 0.5f
                    "low" -> 0.75f
                    ...
                    "high" -> 1.25f
                    "x-high" -> 1.5f
                    else -> throw IllegalArgumentException("Use semitones or a named pitch")
                }
        require(number.isFinite() && number in 0.1f..4f) { "Pitch outside supported range" }
        return number
    }

    private fun volume(value: String): Float =
        when (value) {
            "silent" -> 0f
            "x-soft" -> 0.2f
            "soft" -> 0.5f
