...
internal class NativePlayback(
    ...
    private data class Job(
        val steps: List<NativeSpeechStep>,
        val done: (SpeechResult) -> Unit,
        var index: Int = 0,
    )

    private val jobs = ArrayDeque<Job>()
    val isBusy: Boolean
        @Synchronized get() = jobs.isNotEmpty()

    @Synchronized
    fun enqueue(steps: List<NativeSpeechStep>, done: (SpeechResult) -> Unit): SpeechResult {
        if (steps.isEmpty()) {
            done(SpeechResult.Success)
    ...
    @Synchronized
    private fun complete(job: Job, index: Int, result: SpeechResult) {
        if (jobs.firstOrNull() !== job || job.index != index) return
        if (result == SpeechResult.Success && ++job.index < job.steps.size) {
            dispatch(job)
            return
        }
        jobs.removeFirst()
        ...
        if (next != null)
            post { synchronized(this) { if (jobs.firstOrNull() === next) dispatch(next) } }
    }
