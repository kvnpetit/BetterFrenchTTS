 ...
 *
 * Each rule maps a [word] to either a simple text alias or an exact IPA transcription. A single
 * word can only have **one** active rule — adding a new rule for the same word replaces the
 * previous one, preventing silent conflicts.
 *
 * ## Usage
 *
 * ```kotlin
