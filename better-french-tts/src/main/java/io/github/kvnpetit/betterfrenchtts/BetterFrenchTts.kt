 ...
 *
 * Selects a French voice and compiles a Kotlin DSL to native Android speech operations. Native
 * playback and strict offline voice selection are the defaults. SSML is opt-in.
 *
 * ## Quick start
 *
 * ```kotlin
...
internal constructor(
    ...
    constructor(
        ...
        context,
        config,
        engineFactory = { engineContext, listener ->
    ...
    private val audioFocus: SpeechAudioFocus =
        SpeechAudioFocus(
            ...
            if (!closed) {
                when {
                    config.focusLossBehavior == FocusLossBehavior.PAUSE_QUEUE &&
                        change != AudioManager.AUDIOFOCUS_LOSS &&
                        queueActive -> pauseQueue()
                    config.focusLossBehavior != FocusLossBehavior.IGNORE -> stop()
                    else -> abandonAudioFocus()
                }
            }
        }
    private val activeUtterances = ConcurrentHashMap.newKeySet<String>()
    private val voiceSelector =
        FrenchVoiceSelector(
            config.preferredVoiceNames,
            config.locale,
            config.offlineOnly,
            config.requireExactLocale,
        )

    private val dictionary = PronunciationDictionary()
    private val preparation = SpeechPreparation(config, dictionary)
    private val nativePlayback =
        NativePlayback(::dispatchNativeStep) { action -> mainHandler.post { action() } }
    ...
    private val speechQueue =
        SpeechQueue(
            ...
            dispatch = ::dispatchQueueItem,
            releaseFocus = ::abandonAudioFocus,
            post = { action -> mainHandler.post { action() } },
            onProgress = { onQueueProgressCallback?.invoke(it) },
            onFinished = { onQueueFinishedCallback?.invoke() },
        )
    private val queueActive: Boolean
        get() = speechQueue.isActive
     ...
     * @property audioFocus Audio focus strategy used while speaking. Defaults to
     *   [AudioFocusMode.DUCK] which lowers other apps' volume during speech.
     * @property onReady Called on the **main thread** once the TTS engine is initialized. Receives
     *   the fully ready [BetterFrenchTts] instance.
     * @property onInitError Called on the **main thread** if TTS initialization fails. Receives the
     *   error status code from [android.speech.tts.TextToSpeech.OnInitListener].
     * @property playbackMode Native text/silences by default; SSML requires engine validation.
     * @property locale Preferred French locale; another French locale may be selected if
     *   unavailable.
     * @property offlineOnly Reject network-required and uninstalled voices when true.
     * @property enginePackage Optional installed engine package (Android may apply engine
     *   fallback).
     ...
     *
     * Audio focus tells other apps (music players, podcasts, etc.) to lower their volume or pause
     * while this library is speaking.
     *
    ...
    private fun setup() {
        ...
        if (
            languageResult < TextToSpeech.LANG_AVAILABLE ||
                bestVoice == null ||
                engine.setVoice(bestVoice) != TextToSpeech.SUCCESS
        ) {
            mainHandler.post {
                if (!closed) config.onInitError?.invoke(TextToSpeech.LANG_NOT_SUPPORTED)
            }
            return
        } else {
            ...
            object : UtteranceProgressListener() {
                override fun onStart(utteranceId: String?) {
                    ...
                    if (onSpeechStart != null)
                        mainHandler.post {
                            if (!closed && playbackEpoch.get() == epoch) onSpeechStart?.invoke(id)
                        }
                }
                ...
                override fun onError(utteranceId: String?, errorCode: Int) {
                    handleEngineError(utteranceId, errorCode)
                }
                ...
                override fun onRangeStart(utteranceId: String?, start: Int, end: Int, frame: Int) {
                    ...
                    mainHandler.post {
                        if (!closed && playbackEpoch.get() == epoch)
                            onWordHighlightCallback?.invoke(
                                WordHighlight(id, adjustedStart, adjustedEnd)
                            )
                    }
                }
            }
    ...
    private fun handleEngineError(utteranceId: String?, code: Int?) {
        ...
        val error =
            SpeechResult.Error(
                "TTS synthesis failed" + (code?.let { " (code=$it)" } ?: ""),
                code,
                id,
            )
        ...
        if (activeUtterances.isEmpty() && !nativePlayback.isBusy) abandonAudioFocus()
        mainHandler.post {
            onSpeechError?.invoke(id)
            onDetailedSpeechError?.invoke(error)
        }
    }
     ...
     * @param preset Prosody preset to apply (rate, pitch, volume). Defaults to
     *   [Config.defaultPreset].
     * @param queueMode [TextToSpeech.QUEUE_FLUSH] (default) to interrupt ongoing speech, or
     *   [TextToSpeech.QUEUE_ADD] to append to the queue.
    ...
    fun speak(
        ...
        val processed = preparation.preprocess(text)
        if (config.playbackMode == PlaybackMode.NATIVE)
            return startNative(
                preparation.wrapInProsody(preset, preparation.textToNodes(processed)),
                queueMode,
            )
        val ssml =
            SsmlRenderer.render(
        ...
        val offset = preparation.computeSsmlTextOffset(preset)

        if (config.autoChunkLongText && ssml.length > MAX_SSML_LENGTH) {
            val chunks = preparation.renderTextChunks(processed, preset)
            return dispatchSsmlChunks(chunks, queueMode, offset)
        }
    ...
    fun speak(
        ...
        if (!isReady) return SpeechResult.NotReady
        cancelQueueIfActive()
        return speakNodes(SpeechBuilder().apply(block).nodes, queueMode)
    }

    /**
     * Speaks DSL content wrapped in the given [preset] prosody.
     *
     * This is a convenience method equivalent to wrapping the entire DSL block inside a
     * [SpeechBuilder.withPreset] call.
     *
    ...
    fun speakWithPreset(
        ...
        if (!isReady) return SpeechResult.NotReady
        cancelQueueIfActive()
        return speakNodes(SpeechBuilder().apply(block).nodes, queueMode, preset)
    }
     ...
     *
     * The coroutine is cancelled-safe: cancelling the job will stop the ongoing speech and clean up
    ...
    suspend fun speakAndAwait(
        ...
        if (config.playbackMode == PlaybackMode.NATIVE)
            return awaitNative(preparation.prepareNativeNodes(nodes), queueMode)
     ...
     *
     * The file is written asynchronously by the TTS engine. This method returns immediately after
    ...
    /**
     * Await the engine's file-completion callback. Cancellation stops this export; partial files
     * are retained.
     */
        ...
        withContext(Dispatchers.Main.immediate) {
            suspendCancellableCoroutine { continuation ->
                ...
                if (continuation.isActive) {
                    ...
                    if (result != SpeechResult.Success && continuation.isActive)
    ...
    private fun startFileSynthesis(
        ...
        if (!isReady) return SpeechResult.NotReady
        if (nativePlayback.isBusy || activeUtterances.isNotEmpty())
            return SpeechResult.Error(
                "Stop playback before file synthesis or use a separate instance"
            )
        val processed = preparation.preprocess(text)
        if (config.playbackMode == PlaybackMode.NATIVE) {
            val plan =
                try {
                    NativeSpeechPlan.compile(
                        preparation.wrapInProsody(preset, preparation.textToNodes(processed)),
                        config.normalization.region,
                        false,
                    )
                } catch (error: IllegalArgumentException) {
                    return SpeechResult.Error(error.message ?: "Invalid speech")
                }
            val spoken = plan.joinToString("") { it.text }
            if (spoken.length > 3900)
                return SpeechResult.Error("File synthesis requires a single bounded text")
            val engine = tts ?: return SpeechResult.NotReady
            val step = plan.firstOrNull() ?: NativeSpeechStep()
            if (
                engine.setSpeechRate(step.rate) != TextToSpeech.SUCCESS ||
                    engine.setPitch(step.pitch) != TextToSpeech.SUCCESS
            )
                return SpeechResult.Error("Engine rejected prosody")
            return dispatchFile(
                spoken,
                Bundle().apply { putFloat(TextToSpeech.Engine.KEY_PARAM_VOLUME, step.volume) },
                file,
                onComplete,
                onRegistered,
            )
        ...
        if (ssml.length > MAX_SSML_LENGTH)
            return SpeechResult.Error("File synthesis exceeds the TTS input limit")
        return dispatchFile(ssml, Bundle(), file, onComplete, onRegistered)
    }

    private fun dispatchFile(
        text: String,
        params: Bundle,
        file: File,
        onComplete: ((SpeechResult) -> Unit)?,
        onRegistered: (String) -> Unit,
    ): SpeechResult {
        ...
        val result =
            try {
                ...
                return SpeechResult.Error(
                    error.message ?: "Cannot synthesize to destination",
                    utteranceId = id,
                )
            }
     ...
     *
     * The [ssml] string should be a complete SSML document (with `<speak>` root) or a fragment that
     * the TTS engine can interpret.
     *
    ...
    /**
     * Returns the SSML that would be generated for [text] with the given [preset], without speaking
     * it.
     *
    ...
    fun trySetVoice(voice: Voice): SpeechResult {
        ...
        if (!isReady) return SpeechResult.NotReady
        if (nativePlayback.isBusy || activeUtterances.isNotEmpty())
            return SpeechResult.Error("Stop playback before changing voice")
        if (
            ...
            return SpeechResult.Error("Voice violates French/offline policy")
        if (config.requireExactLocale && voice.locale != config.locale)
            return SpeechResult.Error("Voice violates exact locale policy")
        if (engine.setVoice(voice) != TextToSpeech.SUCCESS)
            return SpeechResult.Error("Engine rejected voice")
        currentVoice = voice
        return SpeechResult.Success
    }

    fun preview(text: String): FrenchTextPreprocessor.Preview =
        if (config.preprocessText) FrenchTextPreprocessor.preview(text, config.normalization)
    ...
    fun previewSpeech(text: String, preset: SpeechPreset = config.defaultPreset): SpeechPreview {
        val normalized = preparation.preprocess(text)
        val nodes = preparation.wrapInProsody(preset, preparation.textToNodes(normalized))
        return try {
            if (config.playbackMode == PlaybackMode.NATIVE)
                SpeechPreview(
                    text,
            ...
            SpeechPreview(
                text,
                normalized,
                result = SpeechResult.Error(error.message ?: "Invalid speech"),
            )
        }
    }
    ...
    fun importPronunciations(data: String, replace: Boolean = false) {
        dictionary.import(data, replace)
    }
     ...
     *
     * Each word can only have **one** active rule. Adding a new rule for the same word replaces the
     * previous one, preventing conflicts between alias and IPA.
     *
     ...
     * ```
     *
     * @param rule The [PronunciationRule] to add (either [PronunciationRule.Alias] or
     *   [PronunciationRule.Ipa]).
     ...
     *
     * Items are not played until [playQueue] is called. Multiple items can be enqueued before
     * starting playback.
     *
     * ```kotlin
    ...
    fun enqueue(text: String, preset: SpeechPreset = config.defaultPreset): BetterFrenchTts {
        speechQueue.add(QueueItem.Text(text, preset))
        return this
    }
    ...
    fun enqueue(block: SpeechBuilder.() -> Unit): BetterFrenchTts {
        speechQueue.add(QueueItem.Dsl(block))
        return this
    }
    ...
    fun enqueueAll(
        ...
        speechQueue.addAll(texts.map { QueueItem.Text(it, preset) })
        return this
    }
     ...
     *
     * Items are spoken sequentially. Use [onQueueProgress] to track progress and [onQueueFinished]
     * to be notified when all items have been spoken.
     *
     * If a queue is already playing, it is restarted from the first item.
     *
     * @return [SpeechResult.Success] if playback started, [SpeechResult.NotReady] if the engine is
     *   not initialized, or [SpeechResult.Error] if the queue is empty.
     * @see enqueue
     * @see pauseQueue
     * @see skipToNext
     */
    fun playQueue(): SpeechResult = speechQueue.playQueue()

    /**
     * Pauses the speech queue.
     *
     * The currently playing utterance is stopped. Call [resumeQueue] to resume from the **same
     * item** (replayed from the start, since Android TTS does not support mid-utterance pause).
     *
     * @return [SpeechResult.Success] if paused, or [SpeechResult.Error] if no queue is active.
     * @see resumeQueue
     */
    fun pauseQueue(): SpeechResult = speechQueue.pauseQueue()
     ...
     *
     * @return [SpeechResult.Success] if playback resumed, or [SpeechResult.Error] if the queue is
     *   not paused.
     * @see pauseQueue
     */
    fun resumeQueue(): SpeechResult = speechQueue.resumeQueue()

    /**
     * Skips to the next item in the queue.
     *
     * The currently playing utterance is stopped and the next item starts immediately. If the
     * current item is the last one, the queue finishes.
     *
     * @return [SpeechResult.Success] if the next item started (or queue finished), or
     *   [SpeechResult.Error] if no queue is active.
     */
    fun skipToNext(): SpeechResult = speechQueue.skipToNext()
    ...
    /**
     * Registers a callback invoked on the **main thread** each time a new queue item starts
     * playing.
     *
     * ```kotlin
     ...
     *
     * Pending [speakAndAwait] calls complete with [SpeechResult.Error]. Cancelling a caller's
     * coroutine stops this instance's playback, including queued native jobs.
     */
    fun stop() {
        tts?.stop()
        clearPlaybackState()
    ...
    /**
     * Receives asynchronous synthesis failures with Android code and utterance ID on the main
     * thread.
     */
    ...
    /**
     * Registers a callback invoked on the **main thread** each time the TTS engine starts speaking
     * a new word or text range.
     *
     * For calls made with [speak]\(text\) or [speakAndAwait]\(text\), the [WordHighlight.start] and
     * [WordHighlight.end] positions have their outer wrapper offset removed. Preprocessing,
     * escaping and chunking can still change source positions; validate these ranges against the
     * displayed text before using them for highlighting.
     *
     * When speech finishes, a final [WordHighlight] with `start = -1` and `end = -1` is emitted to
     * signal that highlighting should be cleared.
     *
     * Native positions refer to the current spoken segment; SSML positions refer to markup. Neither
     * mode promises an exact map to the original source string.
     *
    ...
    /**
     * Releases all TTS resources. **Must** be called when the instance is no longer needed (e.g. in
     * `Activity.onDestroy()` or a Compose `DisposableEffect`).
     *
     * After calling this method, [isInitialized] returns `false` and all pending callbacks are
     * cleared.
     */
    fun shutdown() {
        closed = true
    ...
    private fun dispatchQueueItem(
        item: QueueItem,
        onItemDone: (SpeechResult) -> Unit,
    ): SpeechResult =
        when (item) {
            ...
            is QueueItem.Dsl -> {
                ...
                if (config.playbackMode == PlaybackMode.NATIVE) {
                    startNative(
                        preparation.prepareNativeNodes(nodes),
                        TextToSpeech.QUEUE_FLUSH,
                        onItemDone,
                    )
                } else {
    ...
    private fun dispatchQueueTextItem(
        item: QueueItem.Text,
        onItemDone: (SpeechResult) -> Unit,
    ): SpeechResult {
        val processed = preparation.preprocess(item.text)
        ...
        if (config.autoChunkLongText && ssml.length > MAX_SSML_LENGTH) {
            val chunks = preparation.renderTextChunks(processed, preset)
            return dispatchSsmlChunks(chunks, TextToSpeech.QUEUE_FLUSH, offset, onItemDone)
        }
    ...
    private fun cancelQueueIfActive() {
        if (queueActive) speechQueue.resetQueueState()
    }

    private fun clearPlaybackState() {
        speechQueue.resetQueueState()
        interruptPlayback()
    }
    ...
    private fun speakNodes(
        nodes: List<SsmlNode>,
        queueMode: Int,
        preset: SpeechPreset? = null,
    ): SpeechResult {
        fun wrap(children: List<SsmlNode>) =
            if (preset == null) children else preparation.wrapInProsody(preset, children)

        if (config.playbackMode == PlaybackMode.NATIVE) {
            return startNative(wrap(preparation.prepareNativeNodes(nodes)), queueMode)
        }
        val ssml = SsmlRenderer.render(wrap(nodes))
        if (!config.autoChunkLongText || ssml.length <= MAX_SSML_LENGTH) {
            return dispatchSsml(ssml, queueMode)
        }
        val overhead = preset?.let(preparation::computeProsodyOverhead) ?: 0
        val chunks =
            SsmlRenderer.chunkNodes(nodes, 3900 - SPEAK_OVERHEAD - overhead).map {
                SsmlRenderer.render(wrap(it))
            }
        return dispatchSsmlChunks(chunks, queueMode)
    }

    private fun dispatchSsmlChunks(
        chunks: List<String>,
        queueMode: Int,
        offset: Int = 0,
        onComplete: ((SpeechResult) -> Unit)? = null,
    ...
    private fun startNative(
        ...
        val steps =
            try {
                ...
                return SpeechResult.Error(error.message ?: "Invalid native speech")
            }
    ...
    private suspend fun awaitNative(nodes: List<SsmlNode>, queueMode: Int): SpeechResult =
        suspendCancellableCoroutine { continuation ->
            continuation.invokeOnCancellation { stop() }
            if (continuation.isActive) {
                val result =
                    startNative(nodes, queueMode) {
    ...
    private fun dispatchNativeStep(
        step: NativeSpeechStep,
        complete: (SpeechResult) -> Unit,
    ): SpeechResult {
        val engine = tts ?: return SpeechResult.NotReady
        if (!isReady) return SpeechResult.NotReady
        if (
            ...
            return SpeechResult.Error("Engine rejected speech controls")
        ...
        val result =
            if (step.silenceMs > 0)
                engine.playSilentUtterance(step.silenceMs, TextToSpeech.QUEUE_ADD, id)
            else
                engine.speak(
                    step.text,
    ...
    private fun dispatchSsml(
        ...
        if (ssml.length > MAX_SSML_LENGTH)
            return SpeechResult.Error("Rendered speech exceeds the TTS input limit")
        if (queueMode == TextToSpeech.QUEUE_FLUSH) {
            tts?.stop()
            interruptPlayback()
        }
    ...
    private suspend fun awaitUtterance(
        ...
        return suspendCancellableCoroutine { cont ->
            ...
            activeUtterances.add(utteranceId)
            if (textOffset > 0) ssmlTextOffsets[utteranceId] = textOffset
            pendingCallbacks[utteranceId] = { result -> if (cont.isActive) cont.resume(result) }
            ...
            if (result != TextToSpeech.SUCCESS) {
                ...
                pendingCallbacks
                    ...
                    ?.invoke(
                        SpeechResult.Error(
                            "TTS speak returned error code: $result",
                            result,
                            utteranceId,
                        )
                    )
    ...
    private fun requestAudioFocus(): Boolean = audioFocus.request()

    private fun abandonAudioFocus(): Unit = audioFocus.abandon()
}
