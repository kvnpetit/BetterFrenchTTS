...
object FrenchFormats {
    ...
    fun cardinal(value: Long, region: FrenchRegion = FrenchRegion.FRANCE): String {
        ...
        if (value < 100) {
            ...
            if (n in 70..79 && !regional)
                return "soixante" + (if (n == 71) " et " else "-") + cardinal(value - 60, region)
            if (n in 80..89 || n >= 90 && !regional) {
                return if (n == 80) "quatre-vingts"
                else "quatre-vingt-" + cardinal(value - 80, region)
            }
            return tens[n / 10] +
                when (n % 10) {
                    0 -> ""
        ...
        if (value < 1000) {
            ...
            return (if (count == 1L) "cent"
            else cardinal(count, region) + " cent" + if (remainder == 0L) "s" else "") +
        ...
        val head =
            if (scale == 1000L) {
                if (count == 1L) "mille"
                    ...
                    cardinal(count, region).let {
                        if (it.endsWith("cents") || it.endsWith("vingts")) it.dropLast(1) else it
                    } + " mille"
            } else
                cardinal(count, region) +
                    (if (scale == 1_000_000L) " million" else " milliard") +
                    if (count > 1) "s" else ""
    ...
    fun number(value: String, region: FrenchRegion = FrenchRegion.FRANCE): String {
        ...
        val integer =
            parts[0].toLongOrNull()
                ?: throw IllegalArgumentException("Number outside supported range")
        val whole =
            if (integer == 0L && parts[0].startsWith('-')) "moins zéro"
            else cardinal(integer, region)
        return whole +
            if (parts.size == 1) ""
            else " virgule " + parts[1].map { small[it.digitToInt()] }.joinToString(" ")
    }

    fun ordinal(
        value: Long,
        feminine: Boolean = false,
        region: FrenchRegion = FrenchRegion.FRANCE,
    ): String {
        ...
        if (
            stem.endsWith("cents") ||
                stem.endsWith("vingts") ||
                stem.endsWith("millions") ||
                stem.endsWith("milliards")
        )
    ...
    private fun feminineCardinal(value: Long, region: FrenchRegion): String {
        ...
        return if (words == "un" || words.endsWith(" un") || words.endsWith("-un")) words + "e"
        else words
    }
    ...
    fun money(
        ...
        require(normalized.matches(Regex("[+-]?\\d+(?:\\.\\d{1,2})?"))) {
            "Use a decimal amount with at most two fractional digits"
        }
        val value = BigDecimal(normalized)
        require(value.scale() <= 2) { "Money supports at most two fractional digits" }
        val names =
            when (currency.uppercase(java.util.Locale.ROOT)) {
                "EUR",
                "€" -> "euro" to "centime"
                "USD",
                "CAD",
        ...
        val wholeWords =
            if (names.first == "livre sterling") feminineCardinal(whole, region)
            else cardinal(whole, region)
        return (if (value.signum() < 0) "moins " else "") +
            wholeWords +
            " " +
            plural(names.first, whole) +
            if (cents == 0) ""
            else
                " et " +
                    cardinal(cents.toLong(), region) +
                    " " +
                    plural(names.second, cents.toLong())
    }
    ...
    fun date(
        ...
        require(pieces.size == 3 && pieces.all { it.matches(Regex("\\d+")) }) {
            "Expected a complete numeric date"
        }
        val order =
            when (format) {
                "dmy" -> listOf(2, 1, 0)
                "ymd" -> listOf(0, 1, 2)
                "mdy" -> listOf(2, 0, 1)
                else -> throw IllegalArgumentException("Use dmy, mdy or ymd")
            }
        require(pieces[order[0]].length == 4) { "Use a four-digit year" }
        val date =
            try {
                LocalDate.of(
                    pieces[order[0]].toInt(),
                    pieces[order[1]].toInt(),
                    pieces[order[2]].toInt(),
                )
            } catch (error: java.time.DateTimeException) {
                throw IllegalArgumentException("Invalid calendar date", error)
            }
        val months =
            listOf(
                "janvier",
        ...
        return (if (date.dayOfMonth == 1) "premier"
        else cardinal(date.dayOfMonth.toLong(), region)) +
            " " +
            months[date.monthValue - 1] +
            " " +
            cardinal(date.year.toLong(), region)
    }
    ...
    fun telephone(value: String, region: FrenchRegion = FrenchRegion.FRANCE): String {
        ...
        if (
            digits.length == 10 &&
                digits.startsWith('0') &&
                !value.startsWith('+') &&
                region == FrenchRegion.FRANCE
        ) {
            return digits.chunked(2).joinToString(", ") {
                if (it.startsWith('0')) "zéro " + small[it[1].digitToInt()]
                else cardinal(it.toLong(), region)
            }
        }
        return (if (value.startsWith('+')) "plus " else "") +
            digits.map { small[it.digitToInt()] }.joinToString(" ")
    }
    ...
    fun duration(totalSeconds: Long, region: FrenchRegion = FrenchRegion.FRANCE): String {
        ...
        val values =
            listOf(
                totalSeconds / 3600 to "heure",
                totalSeconds / 60 % 60 to "minute",
                totalSeconds % 60 to "seconde",
            )
        return values
            .filter { it.first != 0L }
            .joinToString(" et ") { (n, unit) ->
