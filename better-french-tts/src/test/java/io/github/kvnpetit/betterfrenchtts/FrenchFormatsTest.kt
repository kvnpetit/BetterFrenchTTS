...
class FrenchFormatsTest {
    @Test
    fun feminineQuantitiesAgreeWithoutChangingInternalThousands() {
    ...
    @Test
    fun quantitiesBelowTwoAndStandardFirstOrdinalAreSingular() {
        assertEquals(
            "0,5 mètre et 1,5 euro et -1 kilogramme",
            FrenchTextPreprocessor.process("0,5 m et 1,5 € et -1 kg"),
        )
        assertEquals("première fois", FrenchTextPreprocessor.process("1re fois"))
    }

    @Test
    fun scientificNotationAndVersionIdentifiersAreNotPartiallyExpanded() {
        val input = "1e3€ 1.2e-3m v1.5€ 1.2.3€"
        assertEquals(input, FrenchTextPreprocessor.process(input))
    }

    @Test
    fun cardinalAgreementsAndScales() {
        mapOf(
                0L to "zéro",
                21L to "vingt et un",
                71L to "soixante et onze",
                80L to "quatre-vingts",
                81L to "quatre-vingt-un",
                91L to "quatre-vingt-onze",
                200L to "deux cents",
                201L to "deux cent un",
                3000L to "trois mille",
                80000L to "quatre-vingt mille",
                200000L to "deux cent mille",
                2000000L to "deux millions",
                -42L to "moins quarante-deux",
            )
            .forEach { (n, word) -> assertEquals(word, FrenchFormats.cardinal(n)) }
    }

    @Test
    fun regionalConventionsAreExplicit() {
    ...
    @Test
    fun ordinalsAreNotLimitedToATable() {
        mapOf(
                1L to "premier",
                5L to "cinquième",
                9L to "neuvième",
                22L to "vingt-deuxième",
                80L to "quatre-vingtième",
                200L to "deux-centième",
                1000L to "millième",
            )
            .forEach { (n, word) -> assertEquals(word, FrenchFormats.ordinal(n)) }
        assertEquals("première", FrenchFormats.ordinal(1, true))
    }

    @Test
    fun decimalsPreserveZeros() {
        assertEquals("moins zéro virgule zéro cinq", FrenchFormats.number("-0,05"))
        assertEquals(
            "mille deux cent trente-quatre virgule cinq zéro",
            FrenchFormats.number("1\u202f234,50"),
        )
    }

    @Test
    fun moneyHasUnitsAndCents() {
    ...
    @Test
    fun explicitDatesValidateCalendar() {
    ...
    @Test
    fun telephonesPreserveLeadingZeros() {
        assertEquals(
            "zéro six, zéro un, zéro deux, zéro trois, zéro quatre",
            FrenchFormats.telephone("06 01 02 03 04"),
        )
        assertTrue(FrenchFormats.telephone("+33 6 01 02 03 04").startsWith("plus trois trois"))
    }

    @Test
    fun durationsRespectGenderAndZero() {
    ...
    @Test
    fun invalidInputsFailWithoutApproximation() {
        assertThrows(IllegalArgumentException::class.java) {
            FrenchFormats.cardinal(Long.MIN_VALUE)
        }
    ...
    @Test
    fun previewExplainsChangesAndProtectsIdentifiers() {
        ...
        assertEquals(
            listOf("abbreviations", "times", "currencies"),
            preview.transformations.map { it.rule },
        )
        ...
        assertEquals(
            "1h",
            FrenchTextPreprocessor.process("1h", FrenchTextPreprocessor.Options(times = false)),
        )
    }
}
