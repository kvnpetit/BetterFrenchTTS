...
class PreprocessingTest {
    @Test fun emptyAndOrdinaryTextStayUnchanged() {
        for (text in listOf("", "Bonjour, été !", "XIV", "Louis XIVabc")) assertEquals(text, process(text))
    }

    @Test fun titlesRespectWordBoundaries() {
    ...
    @Test fun unitsHandleSingularPluralAndCompoundNames() {
    ...
    @Test fun currenciesAndPercentagesAreExpanded() {
        assertEquals("15 euros et 50 pourcent", process("15€ et 50%"))
    }

    @Test fun centuriesUseFrenchOrdinals() {
        assertEquals("premier siècle, quatrième siècle, vingtième siècle", process("Ie siècle, IVe siècle, XXe siècle"))
    }
}
