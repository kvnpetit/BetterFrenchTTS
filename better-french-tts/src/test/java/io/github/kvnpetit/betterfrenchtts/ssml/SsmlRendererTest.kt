...
class SsmlRendererTest {
    ...
    fun longNestedTextIsSplitIntoValidBoundedXmlWithoutLosingContent() {
        ...
        val nodes =
            listOf(
                SsmlNode.Paragraph(
                    listOf(SsmlNode.Prosody(rate = "slow", children = listOf(SsmlNode.Text(text))))
                )
            )
    ...
    fun emptyNodesAndUnstyledProsodyRenderWithoutExtraTags() {
        ...
        assertEquals(
            "<speak>bonjour</speak>",
            SsmlRenderer.render(
                listOf(SsmlNode.Prosody(children = listOf(SsmlNode.Text("bonjour"))))
            ),
        )
    }
    ...
    fun chunkingMeasuresEscapedLength() {
        ...
        assertEquals(
            listOf(listOf(node), listOf(node)),
            SsmlRenderer.chunkNodes(listOf(node, node), 5),
        )
    }
            ...
            listOf(
                SsmlNode.Paragraph(
                    listOf(
                        SsmlNode.Prosody(
                            rate = "slow",
                            children = listOf(SsmlNode.Text("Bonjour")),
                        ),
                        SsmlNode.Break(200),
                    )
                )
