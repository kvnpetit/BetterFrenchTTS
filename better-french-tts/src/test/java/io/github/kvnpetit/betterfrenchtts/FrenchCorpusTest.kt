...
class FrenchCorpusTest(private val input: String, private val expected: String) {
    companion object {
        ...
        fun cases(): Collection<Array<String>> {
            val stream = requireNotNull(FrenchCorpusTest::class.java.getResourceAsStream("/french-corpus.tsv"))
            return stream.bufferedReader(Charsets.UTF_8).useLines { lines ->
                lines.filter { it.isNotBlank() && !it.startsWith('#') }.map { line ->
                    val fields = line.split('\t'); require(fields.size == 2); arrayOf(fields[0], fields[1])
                }.toList().also { require(it.size == 200) }
            }
        }
    }
    @Test fun normalizationMatchesReference() { assertEquals(expected, FrenchTextPreprocessor.process(input)) }
}
