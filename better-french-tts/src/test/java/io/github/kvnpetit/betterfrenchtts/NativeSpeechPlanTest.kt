...
class NativeSpeechPlanTest {
    @Test
    fun adjacentIdenticalStylesAndAliasesStayInOneUtterance() {
        val steps = plan {
            text("Les ")
            sub("amis", "amis")
            prosody(rate = "medium") { text(" arrivent.") }
        }
    ...
    @Test
    fun explicitSilenceAndDifferentControlsAreNotMerged() {
        val steps = plan {
            text("a")
            pause(25)
            text("b")
    ...
    @Test
    fun mergedFragmentsStillRespectLengthAndUnicodeLimits() {
    ...
    private fun plan(block: SpeechBuilder.() -> Unit) =
        NativeSpeechPlan.compile(SpeechBuilder().apply(block).nodes, FrenchRegion.FRANCE)

    @Test
    fun textIsNotXmlAndAliasIsSpokenDirectly() {
        assertEquals(
            listOf("A & <B> Oua-ouei"),
            plan {
                    text("A & <B> ")
    ...
    @Test
    fun pausesAndProsodyBecomeNativeControls() {
        val steps = plan {
            slow {
                text("lent")
                pause(300)
    ...
    @Test
    fun unsupportedIpaAndInvalidControlsFailExplicitly() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            plan { prosody(rate = "NaN%") { text("x") } }
        }
    }

    @Test
    fun spellingNormalizesAccentsAndPreservesCodePoints() {
    ...
    @Test
    fun longAliasesAreSplitWithoutMarkup() {
