...
there is no `version.txt` to keep in sync. This release checkout defaults to `2.1.2`.

## Prepare a release

1. Keep changes in `CHANGELOG.md` under `## [Unreleased]` during development.
2. Choose a new, unused version (this release checkout is **2.1.2**).
...
```sh
./gradlew checkKotlinFormat assemble testDebugUnitTest lint :better-french-tts:dokkaGeneratePublicationHtml :better-french-tts:publishToMavenLocal '-Pversion=2.1.2'
...
```kotlin
implementation("com.github.kvnpetit:better-french-tts:v2.1.2")
```
